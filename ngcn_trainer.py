...
flags.DEFINE_string('dataset_dir',
                    'C:\\Roy\\MSc\\Courses\\Semester 2 2022\\Introduction to DL\\Project\\Codedata\\planetoid-master\\data',
...
flags.DEFINE_integer('hidden_dim', '10',
...
flags.DEFINE_string('adj_pows', '0,1,2',
...
flags.DEFINE_integer('early_stop_steps', 200, 'If the validation accuracy does '
                                             'not increase for this many steps, training is halted.')
flags.DEFINE_float('l2reg', 1e-3, 'L2 Regularization on Kernels.')
...
flags.DEFINE_float('learn_rate', 1, 'Learning Rate for the optimizer.')
...
def main(unused_argv):
    ...
    print(type(accuracy_monitor.params_at_best))
    print(len(accuracy_monitor.params_at_best))
    #print(accuracy_monitor.params_at_best)

    '''
    dataTSNE = list(accuracy_monitor.params_at_best.values())
    dataTSNE = np.array(dataTSNE, dtype=object)


    # Apply t-SNE for dimensionality reduction
    tsne = TSNE(n_components=2, random_state=42)
    embedding = tsne.fit_transform(dataTSNE)
...
######## show plt of cora network dataset ###########

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    data_dir = "C:\\Users\\segev\\Desktop\\data\\planetoid-master\\data"
