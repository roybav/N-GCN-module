...
def adj_times_x(adj, x, adj_pow=1):
    ...
    print("adj_times_x im here:",adj_pow )

    if (adj_pow!=2):

        for i in range(adj_pow):
            print("adj_times_x im here in loop:", i)
            x = tf.sparse_tensor_dense_matmul(adj, x)
    else:
        adj_ordered = tf.sparse.reorder(adj)  # Reorder the sparse tensor indices
        adj_dense = tf.sparse.to_dense(adj_ordered)  # Convert reordered sparse adj to dense tensor
        x = tf.linalg.expm(adj_dense)  # Calculate matrix exponential
